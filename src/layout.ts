...
export function nav(): string {
  ...
  return /*html*/`
    <nav class="flex items-center gap-6 px-6 py-4 border-b border-gray-200 text-sm">
      ...
      <a href="${lp('catalogue')}" class="text-gray-600 hover:text-green-700">${t('nav.catalogue')}</a>
      <span class="flex-1"></span>
      <a href="${lp('privacy')}" class="text-gray-400 hover:text-gray-600 text-xs">${t('nav.privacy')}</a>
      <a href="${lp('legal')}" class="text-gray-400 hover:text-gray-600 text-xs">${t('nav.legal')}</a>
